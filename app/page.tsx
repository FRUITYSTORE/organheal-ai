  ...
  {isArabic
    ...
    : "Note: this stage only presents subscription value. Payments and real feature gating will be added later after the plans page is approved."}
      ...
      >
        ...
        >
          <div>
            <h3 style={{ marginBottom: "10px" }}>OrganHeal AI</h3>

            <p style={{ opacity: 0.78, lineHeight: 1.7, margin: 0 }}>
              {isArabic
                ? "نظام ذكاء صحي شخصي يساعدك على فهم التقييمات الصحية، التقارير الطبية، ونتائج المختبر بطريقة أوضح وأكثر تنظيمًا."
                : "A personal health intelligence system designed to help people understand assessments, medical reports, and lab results more clearly."}
            </p>
          </div>
          ...
          <div>
            ...
            >
              <Link href="/assessment">
                {isArabic ? "التقييم الصحي" : "Health Assessment"}
              </Link>
...
<Link href="/assistant">
  {isArabic ? "المساعد الصحي" : "Health Assistant"}
          ...
          <div>
            <h3 style={{ marginBottom: "10px" }}>
              {isArabic ? "عن OrganHeal" : "About OrganHeal"}
            </h3>
            ...
            >
              ...
              <span>{isArabic ? "التثقيف الصحي" : "Health education"}</span>
              <span>{isArabic ? "متابعة صحية مستقبلية" : "Future follow-up"}</span>
              <span>{isArabic ? "الخصوصية — قريبًا" : "Privacy — coming soon"}</span>
              <span>{isArabic ? "الشروط — قريبًا" : "Terms — coming soon"}</span>
              <span>{isArabic ? "التواصل — قريبًا" : "Contact — coming soon"}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <p style={{ opacity: 0.78, lineHeight: 1.7, margin: 0 }}>
              {isArabic
                ? "OrganHeal AI يقدم معلومات صحية تعليمية وتنظيمية فقط. لا يقدم تشخيصًا، علاجًا، أو نصيحة طبية طارئة، ولا يستبدل الطبيب أو الرعاية الطبية المرخصة."
        ...
        >
          <span>
            {isArabic
              ? "© OrganHeal AI. جميع الحقوق محفوظة."
              : "© OrganHeal AI. All rights reserved."}
          </span>

          <span>
            {isArabic
              ? "مصمم لدعم فهم أفضل للصحة، وليس لاستبدال الطبيب."
              : "Built to support better health understanding, not replace clinicians."}
          </span>
        </div>
      </footer>
    </main>
